...
export function renderMobileMenu(unifiedTree, container) {
  ...
  // Append contact & quick order items to match mobile drawer design (Image 3)
  const contactItems = [
    { name: '1 (800) 233-5469', href: 'tel:18002335469', iconClass: 'icon-phone' },
    { name: 'ocusoft@ocusoft.com', href: 'mailto:ocusoft@ocusoft.com', iconClass: 'icon-mail' },
    { name: 'Quick Order', href: '/quick-order', iconClass: 'icon-cart' },
  ];

  contactItems.forEach((cItem) => {
    const cLi = document.createElement('li');
    cLi.className = 'mobile-nav__item mobile-nav__item--contact';
    const cA = document.createElement('a');
    cA.href = cItem.href;
    cA.innerHTML = `<span class="${cItem.iconClass}"></span> ${cItem.name}`;
    cLi.appendChild(cA);
    rootUl.appendChild(cLi);
  });
